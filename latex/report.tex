...
It's believed that LDP has no API support for SKOS specific resources.
...
% 3 
  ...
  SiSSVoc as a standalone application has no direct support for the creation and update
  of vocabulary terms.  This is inherent to SISSVOC design, as a lightweight web-api
  implemented over a read-only SPARQL endpoint/interface.  

  According to the ANDS handback… ANDS support the editing SKOS files at file
level.  Some support for web-based versioning and
...
Examples are given in the source example.
  ...
  SISSVOC authors advise using a an external vocabulary content maintained using rdf
editors (such as Protege or TopBraid Composer ). which ensure consistency of
relationships between resources is maintained. For web-based - maintenance environment 
  - Enterprise Vocabulary NET and PoolParty Thesaurus Server. 
