...
registrars, not the SKOS contents maintained by particular registrars.  
...
which perform the background work.  SISSVOC uses a SPARQL endpoint as the
mechanism to access RDF providing a strong separation of concerns.

  It would be possible to map the current \texttt{control\_vocab\_db} to expose a sparql
...
unlikely to expose such low-level query capability to their interfaces.
