...
controlled vocabularies using RDF.
  ...
  functionality. However, SISSVoc provides
  no direct support URI patterns for the search and discovery of such content. As an
  alternative, a SPARQL interface does provide a
  query/search mechanism for non-SKOS content such as DC, RDFS or OWL classes. A local-instance
  of SISSVoc could be modified to use this SPARQL with support in the GUI if such 
  functionality was deemed important. 
...
\subsection{ 
  ...
  \item SISSVoc provides an easy-to-use Web-client interface for read-only access to vocabularies and their terms.

  \item As already noted SISSVoc has a read-only application design.
