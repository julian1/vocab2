...
% 1 
  ...
  AODN or EMII dns based url. Also see \footnote{
...
\subsection{Resolvable content should be structured (or at least be able to be queried)
  ...
  The SISSVoc web-service API supports URI patterns for SKOS Concept,
ConceptScheme and Collection. Further URI patterns are provided to discover broader and
narrower terms in transitive and non-transitive forms and according to text
based labels. 

  It's assumed that content can be further decorated with other RDF based
content and persisted. However there is no direct support for searching such content using SISSVoc
URIs patterns. A sparql interface would provide a query/search mechanism for non-SKOS
content such as OWL classes. 
