...
note. 
  SKOS discussion, is SKOS vocabulary items about particular registries, not the
  SKOS contents of any particular registries.  

\subsection{ 
  5. There should ideally be a service interface that is REST-based* and a SPARQL service end-point.
}

  SISSVoc is designed with a HTTP-based interface aligned with REST-based web services. The
  URI patterns facilitate SKOS discovery and access. Note however, that SISSVoc is not a full RESTful API, as
  it does not support HTTP operations for update and deletion of resources.
  
  LDP provides is designed RESTFUL management of registry resources.  
    \footnote { https://github.com/UKGovLD/ukl-registry-poc/wiki/Api } 
